bega <- function(data, reg, cor,...) {
  ...
  #}  
  # Get the communities
  sample_communities <- fit$Monitor[,{{{ncol(fit$Monitor)/3}*2}+1}:ncol(fit$Monitor)]
  ...
  #                  "full_output"=fit)
  #} else {
    Results <- list("R_hat"=R_hat, "Rho_hat"=Rho_hat, "wc"=communities,
                    "sample_R_hat"=sample_R_hat,
                    "sample_Rho_hat"=sample_Rho_hat,
