...
//==================== Functions for the polychoric likelihood
...
std::vector<std::vector<int>> joint_frequency_table(const std::vector<int>& X, const int nobs, const int max_X,
...
double fpoly(double p, const std::vector<double>& tau1, const std::vector<double>& tau2, const std::vector<std::vector<int>>& n,
    const size_t s1, const size_t s2, const std::vector<double>& mvphi1, const std::vector<double>& mvphi2) {

    double f = 0.0;
    for (size_t i = 0; i < s1; ++i) {
        for (size_t j = 0; j < s2; ++j) {
            f += n[i][j] * std::log(pbinorm(tau1[i], tau2[j], tau1[i + 1], tau2[j + 1], p,
...
Rcpp::List dpoly(const arma::mat& X, arma::mat R, std::vector<std::vector<double>> taus) {
    ...
    if (taus.size() != q) Rcpp::stop("The number of tau vectors shuld equal the number of variables");

    std::vector<std::vector<int>> cols(q);
    std::vector<int> maxs(q);
    std::vector<size_t> s(q);
    std::vector<std::vector<double>> mvphi = taus;

    for (size_t i = 0; i < q; ++i) {
        for (int j = 0; j < mvphi[i].size(); ++j) {
            mvphi[i][j] = MVPHI(taus[i][j]);
        }
    ...
    arma::mat loglik(q, q, arma::fill::eye);
    ...
    for (size_t i = 0; i < (q - 1L); ++i) {
        for (int j = (i + 1L); j < q; ++j) {
            tabs[k] = joint_frequency_table(cols[i], nobs, maxs[i], cols[j], maxs[j]);
            loglik(i, j) = loglik(j, i) = fpoly(R(i, j), taus[i], taus[j], tabs[k],
                s[i], s[j], mvphi[i], mvphi[j]) / nobs;
            ++k;
        }
    }
...
//==================== Euclidean distance and partial correlation
...
arma::mat pcor(arma::mat R) {
  ...
  arma::mat Inv = -(V * M.t());
  arma::colvec IS(R.n_rows);
  for(int i = 0; i < int(R.n_rows); i++) {
    Inv(i,i) = -Inv(i,i);
    IS(i) = sqrt(1/Inv(i,i));
  }
  arma::mat pcorM = diagmat(IS) * Inv * diagmat(IS);
  for(int i = 0; i < int(R.n_rows); i++) {
    pcorM(i,i) = 1.0;
  }
  return pcorM;
...
double Modularity(NumericMatrix adjMatrix, IntegerVector community) {
    ...
    m /= 2.0; // Divide by 2 to account for double counting

    double modularity = 0.0;

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            int deltaCommunity = (community[i] == community[j]) ? 1 : 0;
            double ki = sum(adjMatrix(i, _));
            double kj = sum(adjMatrix(j, _));

            modularity += (adjMatrix(i, j) - ki * kj / (2.0 * m)) * deltaCommunity;
        }
    }
